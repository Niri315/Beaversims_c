...
namespace Beaversims.Core.Sim
{
    ...
    internal class GearSet : IEnumerable<KeyValuePair<ItemSlot, GainItem?>>
    {
        ...
        public List<StatBuff> SimStatBuffs { get; set; } = new List<StatBuff>();
        ...
        public void AddSimStatBuff(StatBuff statBuff, double timestamp)
        {
            if (!statBuff.AllowMultiple)
            {
              
                foreach (var curStatBuff in SimStatBuffs)
                {

                    if (statBuff.Id == curStatBuff.Id)
                    {
                        curStatBuff.BuffEnd = timestamp + statBuff.Duration;
                        for (int s = 0; s < statBuff.StatMods.Count; s++)
                        {
                            var statName = statBuff.StatMods[s].StatName;
                            SimIncRatings[statName] -= curStatBuff.StatMods[s].Amount;
                            SimIncRatings[statName] += statBuff.StatMods[s].Amount;
                            curStatBuff.StatMods[s].Amount = statBuff.StatMods[s].Amount;
            ...
            statBuff.BuffEnd = timestamp + statBuff.Duration;
            SimStatBuffs.Add(statBuff);
            foreach (var statMod in statBuff.StatMods)
            {
                SimIncRatings[statMod.StatName] += statMod.Amount;
            }
           
        }
        public void CheckSimStatBuffs(double timestamp)
        {
            for (int i = SimStatBuffs.Count - 1; i >= 0; i--)
            {
                var statBuff = SimStatBuffs[i];

                if (statBuff.BuffEnd < timestamp)
                {
                    foreach (var statMod in statBuff.StatMods)
                    {
                        SimIncRatings[statMod.StatName] -= statMod.Amount;
       ...
       public static void AddSpecialEffects(User user)
        ...
        public static void AddAltAbilityStuff(User user)
        {
            ...
            foreach (var ability in user.Abilities)
            {
                ...
                {
                    ability.Gains.AddRange(
                    Enumerable.Range(0, user.AltGearSets.Count)
                              .Select(_ => Utils.InitGainDict())
                              );
                }
        ...
        public static void CreateTrinketTest(User user, int testIlvl, int itemId)
        {
            var trinketTest = DeepCloneGearset(user.Gear);
            var trinketName = ItemDatabase.Items[itemId].Name;
            trinketTest.Name = trinketName;
            trinketTest[ItemSlot.Trinket1] = ItemGenerator.CreateItem(itemId, testIlvl, ItemSlot.Trinket1, []);
            trinketTest[ItemSlot.Trinket2] = null;
        ...
        public static void TrinketCompare(User user)
        {
            ...
            var testIlvl = 259;
            var trinketTestBaseLine = DeepCloneGearset(user.Gear);
            trinketTestBaseLine.Name = "Null Trinkets";
            trinketTestBaseLine[ItemSlot.Trinket1] = null;
            trinketTestBaseLine[ItemSlot.Trinket2] = null;
            user.AltGearSets.Add(trinketTestBaseLine);


            CreateTrinketTest(user, testIlvl, 251788);
            CreateTrinketTest(user, testIlvl, 251789);
            CreateTrinketTest(user, testIlvl, 249809);
            CreateTrinketTest(user, testIlvl, 250256);
            CreateTrinketTest(user, testIlvl, 249341);
            CreateTrinketTest(user, testIlvl, 250258);
            CreateTrinketTest(user, testIlvl, 249343);
            CreateTrinketTest(user, testIlvl, 251787);
            CreateTrinketTest(user, testIlvl, 248583);
            CreateTrinketTest(user, testIlvl, 255613);
            CreateTrinketTest(user, testIlvl, 255614);
            CreateTrinketTest(user, testIlvl, 264701);
            CreateTrinketTest(user, testIlvl, 249341);
            CreateTrinketTest(user, testIlvl, 264694);
            CreateTrinketTest(user, testIlvl, 250248);
            CreateTrinketTest(user, testIlvl, 250255);
        }
