...
function computeBonusIds(inst) {
    ...
    if (inst.tertiary && BONUS_IDS.tertiary[inst.tertiary]) {
        extra.push(BONUS_IDS.tertiary[inst.tertiary]);
    }
    if (inst.craftedStats && BONUS_IDS.crafted[inst.craftedStats]) {
        extra.push(BONUS_IDS.crafted[inst.craftedStats]);
    }

    const base = Array.isArray(inst.bonusIDs) ? inst.bonusIDs : [];
    inst.bonusIDs = Array.from(new Set([...base, ...extra])).sort((a, b) => a - b);
...
function renderItemCard(inst, selected, section, selection, sections, notify) {
    ...
    card.addEventListener("click", (e) => {
        if (e.target.closest(".tg-edit") || e.target.closest(".tg-copy") || e.target.closest(".tg-remove") || e.target.closest("a")) return;
...
function buildDirectiveFromSelection(selection, sections) {
    ...
    for (const [slot, choices] of normalChoices.entries()) {
        dims.push(choices.map(itemId => ({ type: "slot", slot, itemId })));
    }
    // Rings dimension: each choice is a pair applied to slots 10 & 11
    dims.push(fingerPairs.map(pair => ({ type: "pair", slots: [10, 11], items: pair })));
    // Trinkets dimension: pairs to slots 12 & 13
    dims.push(trinketPairs.map(pair => ({ type: "pair", slots: [12, 13], items: pair })));
    ...
    for (const pick of product(dims)) {
        ...
        for (const p of pick) {
            if (p.type === "slot") {
                ...
                (slots[key] ||= []).push(enrichItem(p.itemId, sections));
            } else if (p.type === "pair") {
                const [s1, s2] = p.slots;
                const [i1, i2] = p.items;
                (slots[String(s1)] ||= []).push(enrichItem(i1, sections));
                (slots[String(s2)] ||= []).push(enrichItem(i2, sections));
            }
        }
        ...
        gearSets.push({
            ...
            name: `Set #${id}`,
            slots,
            enchants: {}, // keeping shape consistent for your wasm layer
            gems: {}
        });
        id++;
    }
...
function computeComboPlan(selection, sections, { countOnly = false } = {}) {
    ...
    for (const slot of NORMAL_SLOTS) {
        ...
        // if nothing selected for that slot, treat as "no choice" (empty choices => zero combos)
        normalChoices.set(slot, items.map(i => i.itemId));
    }

    // FINGERS/TRINKETS → unordered pairs (choose 2) as arrays of [itemIdA, itemIdB]
    const fingerItems = (selectedByKey.get("FINGERS") || []).map(i => i.itemId);
    const trinketItems = (selectedByKey.get("TRINKETS") || []).map(i => i.itemId);
